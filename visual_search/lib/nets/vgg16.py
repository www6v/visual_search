...
class vgg16(object):
  ...
  def create_architecture(self, sess, mode, num_classes, 
    ...
    print self._caffe_weight_path

    with h5py.File(self._caffe_weight_path, 'r') as f1:
      self._caffe_layers = np.load(f1).tolist();     

    #with open(self._caffe_weight_path, 'r') as f:
      #self._caffe_layers = pickle.load(f)
      #self._caffe_layers = np.load(f).tolist();
    print 'Done!'
